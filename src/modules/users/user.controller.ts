...
export class UserController {
  ...
  assignCampaigns = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const payload = z
        .object({ assigned_promotions: z.array(z.string()).optional() })
        .parse(req.body);
  ...
  createUser = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const newUserData: {
        ...
        ...(validatedData.phone_number !== undefined && {
          phone_number: validatedData.phone_number,
        }),
        ...(validatedData.prize_name !== undefined && {
          prize_name: validatedData.prize_name,
        }),
        ...(validatedData.prize_type !== undefined && {
          prize_type: validatedData.prize_type,
        }),
        ...(validatedData.prize_date !== undefined && {
          prize_date: validatedData.prize_date,
        }),
        ...(validatedData.campaign !== undefined && {
          campaign: validatedData.campaign,
        }),
        ...(validatedData.assigned_promotions !== undefined && {
          assigned_promotions: validatedData.assigned_promotions,
        }),
      };
  ...
  /** 
   * POST /api/users/impersonate/:userIdToImpersonate
   * Impersonate another user (Admin only)
   */
  impersonateUser = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      if (!req.user) {
        ResponseBuilder.error(res, "Authentication required", 401);
        return;
      ...
      const userIdToImpersonate = z
        .string()
        .min(1)
        .parse(req.params.userIdToImpoersonate);

      const impersonatedUser = await userService.impersonateUser(
        req.user.id,
        userIdToImpersonate
      );
